...
This nested crate proves that heavy and infrequent routes from `embedded-alerts/eal-leptos-web` can be built and deployed independently of the main Leptos web server. It does not change the existing server entrypoint, initialize Leptos, open a listener, create a WebSocket, or construct the main application/database state.

## Entry points

| Binary | API Gateway route | Required body field | Intended workload |
|---|---|---|---|
| `heavy_alert_batch` | `POST /api/heavy/alert-batch` | `alert_ids` | Fan out a large alert batch independently of Leptos SSR. |
| `heavy_delivery_report` | `POST /api/heavy/delivery-report` | `campaign_id` | Aggregate a delivery report in an isolated function. |
